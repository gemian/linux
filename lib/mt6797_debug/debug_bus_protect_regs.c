...
void __init mt6797_debug_bus_protect_regs_init(struct dentry *regs_dir) {
	struct dentry *dir;
	dir = debugfs_create_dir("bus_protect", regs_dir);
	PRINT_IF_ERROR(dir);
	PRINT_IF_ERROR(REGISTER_FILE(dir, INFRACFG_BASE, INFRA_TOPAXI_PROTECT_EN));
	PRINT_IF_ERROR(REGISTER_FILE(dir, INFRACFG_BASE, INFRA_TOPAXI_PROTECT_STA0));
	PRINT_IF_ERROR(REGISTER_FILE(dir, INFRACFG_BASE, INFRA_TOPAXI_PROTECT_STA1));
}
